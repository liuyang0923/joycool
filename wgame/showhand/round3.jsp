<%@ page contentType="text/vnd.wap.wml;charset=utf-8"%><%@ page import="net.joycool.wap.framework.BaseAction"%><%@ page import="net.joycool.wap.action.wgame.ShowHandAction"%><%@ page import="net.joycool.wap.bean.wgame.*"%><%@ page import="net.joycool.wap.util.*"%><?xml version="1.0" encoding="UTF-8"?>
...
if(sh.getFlag() == 0){
%>
对家下注<%=sh.getRoundWager()%>个乐币！<br/>
<a href="round3.jsp?action=follow">跟了</a>|<a href="round3.jsp?action=quit">放弃</a>|<a href="round3.jsp?action=add&amp;wager=<%=cWager%>">梭哈</a><br/>
<input type="text" name="wager" format="*N" maxlength="10" value="<%=sh.getRoundWager()%>" title="下注"/><br/>
<anchor title="加注">加注
    <go href="round3.jsp" method="post">
...
%>
<input type="text" name="wager" format="*N" maxlength="10" value="<%=sh.getRoundWager()%>" title="下注"/><br/>
<anchor title="下注">下注<go href="round3.jsp" method="post"><postfield name="wager" value="$wager"/><postfield name="action" value="add"/></go></anchor>|<a href="round3.jsp?action=add&amp;wager=<%=cWager%>">梭哈</a>|<a href="round2.jsp?action=quit">放弃</a><br/>
<%
}
%>
提示:<br/>
这局您还有<%=cWager%>个乐币<br/>
当前台面上的乐币是<%=(sh.getTotalWager() * 2)%>个.<br/>
