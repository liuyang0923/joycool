/*
 * Created on 2005-12-7
 *
 */
package net.joycool.wap.action.ebook;

import java.io.BufferedReader;
import java.io.File;
import java.io.FileInputStream;
import java.io.InputStreamReader;
import java.util.Vector;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import net.joycool.wap.bean.CatalogBean;
import net.joycool.wap.bean.UserBean;
import net.joycool.wap.bean.ebook.EBookBean;
import net.joycool.wap.bean.ebook.FileNameBean;
import net.joycool.wap.cache.util.UserBagCacheUtil;
import net.joycool.wap.service.factory.ServiceFactory;
import net.joycool.wap.service.infc.ICatalogService;
import net.joycool.wap.service.infc.IEBookService;
import net.joycool.wap.util.Constants;

import org.apache.struts.action.Action;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;

/**
 * @author lbj
 *
 */
public class EBookDownloadListAction extends Action {
    String realUrl = Constants.EBOOK_FILE_URL;
    
    public ActionForward execute(ActionMapping mapping, ActionForm form,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
    	UserBean loginUser=(UserBean)request.getSession().getAttribute(Constants.LOGIN_USER_KEY);
        ICatalogService catalogService = ServiceFactory.createCatalogService();
        File processFile;
        String filesName[];
        String result = null;
		String tip = null;
        String idStr = request.getParameter("ebookId");
        int id;
        if (idStr != null) {
            id = Integer.parseInt(idStr);
        } else {
            return mapping.findForward("systemFailure");
        }
        //用户没登录直接跳转提示页面
        if(loginUser!=null){
        	//用户没有下载卡跳转提示页面
 		    int temp = UserBagCacheUtil.getUserBagById(60, loginUser.getId());
 			if(temp>0)
 			{
// 				取得当前电子书及其所属类别
 		        IEBookService ebookService = ServiceFactory.createEBookService();
 		        String condition = "id = " + id ;
 		        EBookBean ebook = ebookService.getEBook(condition);
 		        int catalogId = ebook.getCatalogId();
 		        String buffCondition = "id = " + catalogId;
 		        CatalogBean catalog = catalogService.getCatalog(buffCondition);
 		        
 		        //对该电子书所在的目录进行操作，得到下载文件或者是阅读文件。
 		        String fileUrl = realUrl + ebook.getFileUrl();
 		        processFile = this.getDirectorys(fileUrl, "jar");
 		        if (processFile == null) 
 		        {
 		        	result = "failure";
 					tip = "对不起，本书不提供下载!(3秒后跳转)";
 					request.setAttribute("result", result);
 					request.setAttribute("tip", tip);
 					return mapping.findForward("success");
 		        }
 		            //return mapping.findForward("systemFailure");
 		        filesName = this.getFiles(fileUrl + "/" + processFile.getName(),"jar");
 		        if (filesName == null)
 		            return mapping.findForward("systemFailure");       
 		        
 		        //对文件名进行分析，得到文件信息。
 		        Vector fileNames = new Vector();
 		        
 		        for (int i = 0; i < filesName.length; i++) {
 		            if (filesName[i] == null)
 		                continue;
 		            FileNameBean fileName = new FileNameBean();
 		            
 		                String names[] = filesName[i].split("_");
 		                if (names == null)
 		                    return mapping.findForward("failure");
 		                fileName.setId(Integer.parseInt(names[1]));
 		                StringBuffer buffer = new StringBuffer();
 		                buffer.append("第");
 		                for (int j = 2; j < names.length; j++) {
 		                    if (names[j].contains("jar")) {
 		                        buffer.append(names[j].substring(0, names[j]
 		                                .indexOf("."))
 		                                + "章");
 		                    } else {
 		                        buffer.append(names[j] + "-");
 		                    }
 		                }
 		                fileName.setChapters(buffer.toString());
 		                fileName.setAddress(Constants.EBOOK_RESOURCE_ROOT_URL
 		                        + ebook.getFileUrl() + "/jar/"
 		                        + filesName[i]);
 		            fileNames.add(fileName);
 		        }
 		        
 		        //更新下载次数
 		        String set = "download_sum = (download_sum + 1)";
 		        condition = "id = " + id;
 		        ebookService.updateEBook(set, condition);
 		        //更新下载卡的使用次数
 		       UserBagCacheUtil.UseUserBagCacheById(loginUser.getId(), temp);
 		        request.setAttribute("catalog",catalog);
 		        request.setAttribute("ebook",ebook);
 		        request.setAttribute("rootId", request.getParameter("rootId"));
 		        request.setAttribute("fileNames", fileNames);
 		         result = "success";
 				request.setAttribute("result", result);
 		        return mapping.findForward("success");
 				
 			}
 			else
 			{
 				return mapping.findForward("prompt");
 			}
        }else
        {
        	return mapping.findForward("prompt");
        }
        
    }
    
    /*
     * get the appointed directory  under the directory;
     */
    private File getDirectorys(String directoryName, String destDirectory) {
        File currentFile = new File(directoryName);
        String allFiles[] = currentFile.list();//get the all files under the
                                              // directory;
        if (allFiles != null) {
            for (int i = 0; i < allFiles.length; i++) {
                if (allFiles[i].equals(destDirectory))
                    return new File(allFiles[i]);
            }
        }
        return null;
    }
    
    /*
     * get the appointed files name under the directory;
     */
    private String[] getFiles(String directoryName,String suffix) {
        File currentFile = new File(directoryName);
        String allFiles[] = currentFile.list();//get the all files under the
        									  // directory;
        if (allFiles != null) {
            int j, k;
            String[] filesNeed = new String[allFiles.length];
            for (int i = 0; i < allFiles.length; i++) {
                if ((allFiles[i].endsWith(suffix))&&(!allFiles[i].startsWith("inf")))  //get the needed file;
                {
                    String index = allFiles[i];
                    j = index.indexOf("_") + 1;
                    k = index.indexOf("_", j);
                    index = index.substring(j, k);
                    int _index = Integer.parseInt(index);
                    filesNeed[_index - 1] = allFiles[i];
                }
            }
            return filesNeed;
        } else
            return null;
    }
    
    
    /**
     * @param directoryName  
     * @return String[]  get the chapters'name from the inf.txt 
     * @throws Exception
     */
    private String[] readInfo(String directoryName) throws Exception{
        File currentFile = new File(directoryName);
        String allFiles[] = currentFile.list();//get the all files under the
        									  // directory;
        if(allFiles != null){
            for(int i = 0;i<allFiles.length;i++){
                if(allFiles[i].equals("inf.txt")){
                    FileInputStream in = new FileInputStream(directoryName + "/" + "inf.txt");
                    InputStreamReader reader = new InputStreamReader(in,"UTF-8");
                    BufferedReader br = new BufferedReader(reader);
                    String buffer = br.readLine();
                    while (!buffer.startsWith("章名")&&buffer != null){
                        buffer = br.readLine();
                    }
                    if(buffer != null){
                        String[] chapters = buffer.split(",");
                        chapters[0] = chapters[0].substring(chapters[0].indexOf(":"));
                        return chapters;    
                    }
                }
            }
        }
        return null;
    }


}
