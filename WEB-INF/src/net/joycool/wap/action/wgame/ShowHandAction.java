...
import net.joycool.wap.bean.wgame.ShowHandBean;
...
//        UserStatusBean status = getUserStatus(loginUser.getId());
    ...
    public void end(HttpServletRequest request) {
        ...
        if (compareCards(sh.getUserCards(), sh.getSystemCards()) > 0) {
            ...
            //加上积分
//          fanys2006-08-11
			UserInfoUtil.updateUserCash(loginUser.getId(),sh.getTotalWager(),UserCashAction.WAGER,"wgame--ShowHandBean--加乐币"+ sh.getTotalWager());
            //zhul_2006-07-25 记录用户的现金流 start
            MoneyAction.addMoneyFlowRecord(Constants.SHOWHAND,sh.getTotalWager(),Constants.PLUS,loginUser.getId());
            ...
            //减去积分
//          fanys2006-08-11
			UserInfoUtil.updateUserCash(loginUser.getId(),-sh.getTotalWager(),UserCashAction.WAGER,"wgame--ShowHandBean--减乐币"+ sh.getTotalWager());
            //zhul_2006-07-25 记录用户的现金流 start
            MoneyAction.addMoneyFlowRecord(Constants.SHOWHAND,sh.getTotalWager(),Constants.SUBTRATION,loginUser.getId());
            //zhul_2006-07-25 记录用户的现金流 end
...
//      fanys2006-08-11
		...
		// mcq_1_更改玩家session中用户乐币数
		updateInfo(loginUser,Constants.RANK_PK_LOSE);
		// mcq_end
        //记录
//        WGameHistoryBean history = new WGameHistoryBean();
//        history.setUserId(loginUser.getId());
//        history.setGameId(WGameHistoryBean.SHOWHAND);
//        history.setResult(WGameHistoryBean.LOSE);
//        history.setRemark("您输了" + sh.getTotalWager() + "个乐币!");
//        history.setGamePoint(sh.getTotalWager());
