...
public final class Base64u {
	...
	public static boolean isArrayByteBase64(byte[] arrayOctect) {
		...
		for (int i = 0; i < length; i++) {
			if (!Base64u.isBase64(arrayOctect[i]))
				return false;
		}
		return true;
	}
	...
	public static synchronized String encodeMd5(String src) {
		try {
	...
	public static synchronized byte[] encodeMd5(byte[] src) {
		digest.update(src);
	...
	public static synchronized byte[] encodeMd5(byte[] src, int off, int len) {
	...
	byte[] output;
	public Base64u(byte[] input) {
		output = encodeMd5(input);
	}	
	public Base64u(byte[] input, int off, int len) {
		output = encodeMd5(input, off, len);
	}
	public long getIndex32() {
		return ((long)output[0] << 24) + ((long)output[1] << 16) + ((long)output[2] << 8) + output[3];
	}
	// base64u编码
	public String get64x() {
