<%@ page contentType="text/vnd.wap.wml;charset=utf-8" %><%@include file="../pageinc.jsp"%><%@ page import="java.util.*,net.joycool.wap.spec.castle.*,net.joycool.wap.framework.*,net.joycool.wap.framework.*,net.joycool.wap.util.*"%><%@ page import="java.util.List"%><%
	...
	int userCur = 0;
	if(cur == -1 && castleUser!=null && castleUser.getTong()!=0){
		userCur = SqlUtil.getIntResult("select id from castle_stat2 where tong_id = " + castleUser.getTong(), 5);
	...
	List list = action.getCastleService().getTongArray(start, limit);
	int count = list.size() > 10 ? 10 : list.size();
%><?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE wml PUBLIC "-//WAPFORUM//DTD WML 1.1//EN" "http://www.wapforum.org/wml_1.1.xml">
<wml><card title="联盟排行榜"><p>
<%if(userCur > 0) {%>
<%} %>==联盟/总人口/成员==<br/>
<%for(int i=0;i<count;i++){
	Object[] obj = (Object[])list.get(i);
%>
<%=obj[0]%>.<a href="../tong/tong.jsp?id=<%=obj[1]%>"><%=StringUtil.toWml((String)obj[3])%></a>/<%=obj[2]%>/<%=obj[4] %>
<br/><%}%>
