<%@ page contentType="text/vnd.wap.wml;charset=utf-8" %><%@include file="../pageinc.jsp"%><%@ page import="java.util.*,net.joycool.wap.spec.castle.*,net.joycool.wap.framework.*,net.joycool.wap.framework.*,net.joycool.wap.util.*"%><%@ page import="java.util.List"%><%
	...
	if(cur == -1 && castleUser!=null){
		userCur = action.getCastleService().getAttackCurArray(castleUser.getUid());
	...
	List list = action.getCastleService().getAttackArray(start, limit);
	int count = list.size() > 10 ? 10 : list.size();
%><?xml version="1.0" encoding="utf-8"?>
<!DOCTYPE wml PUBLIC "-//WAPFORUM//DTD WML 1.1//EN" "http://www.wapforum.org/wml_1.1.xml">
<wml><card title="攻击排行榜"><p>
<%if(userCur > 0) {%>
...
<br/><%}%>
<%if(list.size() > 10) {%><a href="stat3.jsp?p=<%=cur+1%>">下一页</a><%}else{%>下一页<%}%>
<%if(cur > 0) {%><a href="stat3.jsp?p=<%=cur-1%>">上一页</a><%}else{%>上一页<%}%><br/>
跳转到:<input name="p" maxlength="3" format="*N"/><anchor>GO<go href="stat3.jsp"><postfield name="p" value="$p"/></go></anchor><br/>
