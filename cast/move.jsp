...
<%if(army!=null&&!army.isEmpty()){%><%if(request.getAttribute("all")==null){%><a href="move.jsp?id=<%=army.getId()%>">返回</a><br/><%}%><%}%>
<%}else{%><%
if(army != null) {
	...
	if(army.getCid()==cid) {
		castle = CastleUtil.getCastleById(army.getAt());
...
%>【军队详细数据】<br/>
<%if(army.getCid()==army.getAt()){%>自己<%}else{ if(mine){%>支援<%}else{%>来自<%}%><a href="search.jsp?x=<%=castle.getX()%>&amp;y=<%=castle.getY()%>"><%=castle.getCastleNameWml()%>(<%=castle.getX()%>|<%=castle.getY()%>)</a><%}%>的军队<br/><%
...
<go href="move.jsp?id=<%=army.getId()%>&amp;t=1" method="post">
<%
for(int i = 1;i<so.length; i++) {
		SoldierResBean soldier = so[i];
		int count = army.getCount(soldier.getType());
...
</go></anchor>|!<a href="move.jsp?id=<%=army.getId()%>&amp;t=2">全部<%if(army.getCid()==cid){%>召回<%}else{%>回派<%}%></a><br/><%}%>
