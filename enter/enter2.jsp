<%@ page contentType="text/vnd.wap.wml;charset=utf-8" session="false"%><%@ page import="net.joycool.wap.bean.UserBean"%><%@ page import="net.joycool.wap.framework.*"%><%@ page import="net.joycool.wap.util.*"%><%
...
if(loginUser == null){
	...
	if(!server.endsWith("xtdsp.com")&&!server.equals("joycool.net")&&!server.equals("ad.joycool.net")&&!SqlUtil.isTest){
		response.sendRedirect("http://joycool.net/enter/enter2.jsp?" + request.getQueryString());
		return;
	}
	if(server.equals("xtdsp.com")){
		response.sendRedirect("http://a.xtdsp.com/enter/enter2.jsp?" + request.getQueryString());
		return;
	}
/*	String ip = request.getRemoteAddr();
	...
	int id = StringUtil.toInt(request.getParameter("uid"));
...
session.setAttribute("cd-fourmSTime", Long.valueOf(System.currentTimeMillis() + 20000));	// 20秒内不能发帖或者回帖
	...
	LogUtil.logLogin("bookmark2:" + loginUser.getId() + ":" + request.getRemoteAddr() + ":" + request.getHeader("User-Agent") + ":" + new CookieUtil(request).getCookieValue("jsid"));
