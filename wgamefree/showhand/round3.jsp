...
<anchor title="加注">加注
    <go href="round3.jsp?unique=<%=unique%>" method="post">
...
%>
<input type="text" name="wager" format="*N" maxlength="10" value="<%=sh.getRoundWager()%>" title="下注"/><br/>
<anchor title="下注">下注<go href="round3.jsp?unique=<%=unique%>" method="post"><postfield name="wager" value="$wager"/><postfield name="action" value="add"/></go></anchor>|<a href="round3.jsp?action=add&amp;wager=<%=cWager%>">梭哈</a>|<a href="round2.jsp?action=quit">放弃</a><br/>
<%
}
%>
提示：这局您还有<%=cWager%>个乐币。当前台面上的乐币是<%=(sh.getTotalWager() * 2)%>个。<br/>
