...
if(matchrunbean==null){
	response.sendRedirect("runindex.jsp");
	return;
}
//刷新
String refreshUrl = ("runing.jsp?id="+matchrunbean.getId()+"&amp;type="+matchrunbean.getType());
...
已有<%=matchrunbean.getPeoplenumber()%>宠物参加(8人自动开始比赛)<br/>
...
<a href="matchact.jsp?task=3&amp;return=<%=order%>">退出比赛</a><br/>
<%}}%>
...
<%}}}%>
<br/>
<a href="runindex.jsp">返回趣味赛跑比赛列表</a><br/>
</p>
</card>
...
<a href="matchact.jsp?task=4">使用:<%=action.STAGE_STRING[playbean[i].getStage()[0]-1]%></a>

<%if((petUser.getId() == playbean[i].getPetid())&&(playbean[i].getStage()[1] != 0)){%>
(<%=action.STAGE_STRING[playbean[i].getStage()[1]-1]%>)
<%}%>|
<%}%>
<%}}}%>
...
<%}else {%>
<card title="长跑比赛" ontimer="<%=response.encodeURL(refreshUrl)%>">
<timer value="200"/>
<p align="left">
<%=BaseAction.getTop(request, response)%>
<%=tip%>
