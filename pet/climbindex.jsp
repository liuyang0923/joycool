...
<a href="/pet/climbmatchact.jsp?task=1&amp;type=4">新开一局比赛</a><br/>
<%}else{%>
...
<a href="/pet/climbmatchact.jsp?task=2&amp;type=<%=matchrunbean.getType()%>&amp;id=<%=matchrunbean.getId()%>">加入比赛</a>
<%}%>
<br/>
...
共<%=matchrunbean.getPeoplenumber()%>玩家
<%if(matchrunbean.getCondition() != 0) {%>
<a href="/pet/climbing.jsp?task=4&amp;type=<%=matchrunbean.getType()%>&amp;id=<%=matchrunbean.getId()%>">观看比赛</a>
<%}%>
<br/>
<%
}else{%>
...
共<%=matchrunbean.getPeoplenumber()%>玩家
<%if(matchrunbean.getCondition() != 0) {%>
<a href="/pet/climbing.jsp?task=4&amp;type=<%=matchrunbean.getType()%>&amp;id=<%=matchrunbean.getId()%>">查看结果</a>
<%}%>
<br/>
<%}}%>
<%
