...
----------------<br/>
<%if(fsUser!=null){%>
您最近高分为：<%=highscore%>元，排名第<%=action.getAttributeInt("count")%>。<br/>
<%}%>
<%
...
for(int i =0 ;i<fsUserList.size();i++){
	...
	最高分:<%=StringUtil.bigNumberFormat(fsUser1.getRecentHighScore())%>元<br/>
<%}%>
<%=PageUtil.shuzifenye(pagingBean, "top.jsp?type=" + type, true, "|", response)%>
<%if(!fsUser.isGameOver()){%>
<a href="/fs/index.jsp">返回游戏</a><br/>
<%}%>
<a href="/fs/topall.jsp?type=<%=type%>">查看总排行榜</a><br/>
