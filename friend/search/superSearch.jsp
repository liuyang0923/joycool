...
<wml>
<card title="高级搜索"><p><%=BaseAction.getTop(request, response)%><%
%><a href="/friend/friendCenter.jsp">交友中心</a>&gt;<a href="searchCenter.jsp">搜索中心</a>&gt;高级搜索<br/>
==高级搜索==<br/>性别:<select name="gender" value="<%=gender%>">
...
星座:<select name="xz"<%if(astro > 0){%> value="<%=astro%>"<%}else if(base != null){%> value="<%=base.getAstro()%>"<%}%>>
...
对方所在城市:<%=creditCity.getCity()%>[<anchor>修改<go href="province.jsp" method="post"><postfield name="type" value="5"/><postfield name="gender" value="$gender"/><postfield name="age" value="$age"/><postfield name="astro" value="$xz"/><postfield name="aim" value="$aim"/></go></anchor>]<br/>  
...
<anchor>搜索<go href="searchResult.jsp" method="post"><postfield name="type" value="5"/><postfield name="gender" value="$gender"/><postfield name="city" value="<%=creditCity.getId()%>"/><postfield name="age" value="$age"/><postfield name="astro" value="$xz"/><postfield name="aim" value="$aim"/></go></anchor>
<br/>&gt;&gt;更多<a href="emotionSearch.jsp">交友</a>.<a href="homeSearch.jsp">城市</a>.<a href="astroSearch.jsp">星座</a>搜索&lt;&lt;<br/>
==热门搜索==<br/><%
...
%>————————<br/>
搜索乐酷友友<br/><input name="id"  maxlength="10" format="*N" value=""/><br/>
