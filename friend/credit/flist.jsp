...
%><%=i+paging.getStartIndex()+1 %>.<a href="navi2.jsp?uid=<%=user.getId()%>"><%if(user.getNickNameWml()==null || "".equals(user.getNickNameWml())){%>游客<%=user.getId()%><%}else{%><%=user.getNickNameWml()%><%}%></a><br/><%
}%><%=paging.shuzifenye("flist.jsp",false,"|",response)%><%	
} else {
%>您还没有好友,不能打分.<br/><%
}
%><a href="navi.jsp">[返回可信度首页]</a><br/><%
} else {
