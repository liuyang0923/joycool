...
<%if("".equals(tip)){
%><%=UserInfoUtil.getUser(userInfo.getUserId()).getGenderText()%>目前的交友可信度为<%=userInfo.getTotalPoint()%>%<br/>信息完整度(40):<%=userInfo.getIntactPoint()%><br/>
<%=topLink%> 
<%switch (type){
case 0:
%><%if(userBase != null && userBase.getPhoto() != null && !"".equals(userBase.getPhoto())){%><img src="<%=CreditAction.ATTACH_URL_ROOT + "/" + userBase.getPhoto()%>" alt="o"/><br/><%}%>
...
break;
}%><%if(userInfo.getModifyTime() != 0){%>资料更新日期:<%=DateUtil.formatDate1(new Date(userInfo.getModifyTime()))%><%}%><br/>
