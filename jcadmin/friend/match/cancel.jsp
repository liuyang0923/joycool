...
List list = MatchAction.service.getMatchUserList(" checked=3 limit " + paging.getCurrentPageIndex() * COUNT_PRE_PAGE + "," + COUNT_PRE_PAGE);
%>
<html>
	<head>
		<title>参赛用户列表</title>
			...
			<% for (int i = 0 ; i < list.size() ; i++){
				...
				if (matchUser != null){
					%><tr>
							...
							<td><a href="cancel.jsp?r=<%=matchUser.getUserId()%>" onclick="return confirm('真的要恢复她的资格吗？')">恢复资格</a></td>
					  </tr><%
				}
			}%>
		</table>
		<%=paging!=null?paging.shuzifenye("cancel.jsp",false,"|",response):""%>
	</body>
</html>