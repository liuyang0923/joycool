...
<%
	...
	List top;
	if(userId<=0)
		top = SqlUtil.getIntsList("select user_id,tong_id,count,mark from jc_tong_city_record where mark=3 and count>200 order by count desc limit 500");
	else
...
<td><%=tong.getTitle()%>(<a href="../tong/search.jsp?tongId=<%=tong.getId()%>"><%=tong.getId()%></a>)</td>
