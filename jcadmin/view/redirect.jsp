...
static void addRedirect(String url, String toUrl){
	if(url == null ||toUrl==null)
		return;
	HashMap newMap = (HashMap)WGameFilter.wap10Map.clone();
	newMap.put(url, toUrl);
	WGameFilter.wap10Map = newMap;
	SqlUtil.executeUpdate("insert into url_redirect set type=0,url='"+StringUtil.toSql(url)+"',to_url='"+StringUtil.toSql(toUrl)+"'", 5);
}
%><%
	...
	if(act==1){
		...
		SqlUtil.executeUpdate("delete from url_redirect where id=" + action.getParameterInt("del"), 5);
		WGameFilter.loadWap10Map();	// 删除了一个，重新载入
		response.sendRedirect("redirect.jsp");
		return;
	} else if(act==4){
		WGameFilter.loadWap10Map();
	}
	List list = SqlUtil.getObjectsList("select id,url,to_url,type from url_redirect order by url", 5);
%>
<html>
<head>
...
%>
<tr><td width="30"><%=obj[0]%></td><td width="300"><%=obj[1]%></td><td width="300"><%=obj[2]%></td><td><%=obj[3]%></td><td><%=module%></td><td width=30 align=center><a href="redirect.jsp?a=3&del=<%=obj[0]%>" onclick="return confirm('确定删除?')">删</a></td></tr>
<%}%>
</table>
<p>
<form action="redirect.jsp?a=2" method="post">
from:<input type="text" name="url" value="">
to:<input type="text" name="url2" value="">
<input type=submit value="添加">
</form>
</p>
<p><a href="redirect.jsp?a=4" onclick="return confirm('确认重新载入？')">重新载入</a></p>
<body>
</html>