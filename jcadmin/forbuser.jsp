...
if(usernamee!=null){
	ForbidUtil.addForbid("chat",new ForbidUtil.ForbidBean(StringUtil.toInt(usernamee),0,bak),interval,adminUser.getName());
}
...
if(nickName!=null){
   ...
   toUser = userService.getUser("nickname = '"+nickName+"'");
		if(toUser!=null){
		//lbj_log_oper_start
	    LogUtil.logOperation(adminUser.getName() + "聊天室封禁用户: " + toUser.getId() + "昵称:" + toUser.getNickName());
	    //lbj_log_oper_end
			ForbidUtil.addForbid("chat",new ForbidUtil.ForbidBean(toUser.getId(),0,bak),interval, adminUser.getName());
		    
		}else{
		    //response.sendRedirect("forbuser.jsp");
		    BaseAction.sendRedirect("/jcadmin/forbuser.jsp", response);
			return;
		}
   }
}
...
if(request.getParameter("delete") != null){
    ...
    ForbidUtil.deleteForbid("chat",deleteId);
	//response.sendRedirect("forbuser.jsp");
	BaseAction.sendRedirect("/jcadmin/forbuser.jsp", response);
	return;
}
...
<body>
<font color=red>封禁发言列表</font>
<form name="buser" action="forbuser.jsp" method="post" onsubmit="return checkform()" >
用户ID:<input name="username" type="text"  size="38" ><br/>
理由:<input name="bak" type="text"  size="38" value="刷屏或者语言不文明">
...
<form name="nickName" action="forbuser.jsp" method="post" onsubmit="return checkform1()" >
...
<%
//显示封禁列表
List forbidList = ForbidUtil.getForbidList("chat");
if(forbidList != null){
...
<td width="10%"><a href="forbuser.jsp?delete=<%=forbid.getValue()%>">删除</a></td>
</tr>
<%
	    i ++;
	}
