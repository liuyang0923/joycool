...
if(usernamee!=null){
	ForbidUtil.addForbid("team",new ForbidUtil.ForbidBean(StringUtil.toInt(usernamee),0,bak),interval,adminUser.getName());
}
...
if(nickName!=null){
   ...
   toUser = userService.getUser("nickname = '"+nickName+"'");
		if(toUser!=null){
		//lbj_log_oper_start
	    LogUtil.logOperation(adminUser.getName() + "圈子封禁用户: " + toUser.getId() + "昵称:" + toUser.getNickName());
	    //lbj_log_oper_end
			ForbidUtil.addForbid("team",new ForbidUtil.ForbidBean(toUser.getId(),0,bak),interval, adminUser.getName());
		    
		}else{
		    //response.sendRedirect("forbuser6.jsp");
		    BaseAction.sendRedirect("/jcadmin/forbuser6.jsp", response);
			return;
		}
   }
}
...
if(request.getParameter("delete") != null){
    ...
    ForbidUtil.deleteForbid("team",deleteId);
	//response.sendRedirect("forbuser6.jsp");
	BaseAction.sendRedirect("/jcadmin/forbuser6.jsp", response);
	return;
}
...
<body>
<font color=red>封禁圈子权限列表</font>
<form name="buser" action="forbuser6.jsp" method="post" onsubmit="return checkform()" >
用户ID:<input name="username" type="text"  size="38" ><br/>
理由:<input name="bak" type="text"  size="38" value="不良行为">
...
<form name="nickName" action="forbuser6.jsp" method="post" onsubmit="return checkform1()" >
...
<%
//显示封禁列表
List forbidList = ForbidUtil.getForbidList("team");
if(forbidList != null){
...
<td width="10%"><a href="forbuser6.jsp?delete=<%=forbid.getValue()%>">删除</a></td>
</tr>
<%
	    i ++;
	}
