...
action.areaTagList(request, response);
...
count = tagList.size();
for(i = 0; i < count; i ++){
	tag = (AreaTagBean) tagList.get(i);
	
%>
    <tr><td><%=tag.getId()%></td><td><%=tag.getName()%></td><td><%=tag.getDisplayOrder()%></td><td><%=tag.getMark() == 0 ? "普通标签" : "<font color=red>热门标签</font>"%></td><td><a href="editAreaTag.jsp?id=<%=tag.getId()%>">编辑</a> <a href="deleteAreaTag.jsp?id=<%=tag.getId()%>" onclick="return confirm('确认删除？')">删除</a></td></tr>
<%
}
%>
</table>
<br/>
添加标签：<br/>
<form action="addAreaTag.jsp" method="post">
