...
if(usernamee!=null){
	ForbidUtil.addForbid("mail",new ForbidUtil.ForbidBean(StringUtil.toInt(usernamee),0,bak),interval,adminUser.getName());
}
...
if(nickName!=null){
   ...
   toUser = userService.getUser("nickname = '"+nickName+"'");
		if(toUser!=null){
		//lbj_log_oper_start
	    LogUtil.logOperation(adminUser.getName() + "信件封禁用户: " + toUser.getId() + "昵称:" + toUser.getNickName());
	    //lbj_log_oper_end
			ForbidUtil.addForbid("mail",new ForbidUtil.ForbidBean(toUser.getId(),0,bak),interval, adminUser.getName());
		    
		}else{
		    //response.sendRedirect("forbuser2.jsp");
		    BaseAction.sendRedirect("/jcadmin/forbuser2.jsp", response);
			return;
		}
   }
}
...
if(request.getParameter("delete") != null){
    ...
    ForbidUtil.deleteForbid("mail",deleteId);
	//response.sendRedirect("forbuser2.jsp");
	BaseAction.sendRedirect("/jcadmin/forbuser2.jsp", response);
	return;
}
...
<body>
<font color=red>封禁信件列表</font>
<form name="buser" action="forbuser2.jsp" method="post" onsubmit="return checkform()" >
用户ID:<input name="username" type="text"  size="38" ><br/>
理由:<input name="bak" type="text"  size="38" value="大量发送不良信件">
...
<form name="nickName" action="forbuser2.jsp" method="post" onsubmit="return checkform1()" >
...
<%
//显示封禁列表
List forbidList = ForbidUtil.getForbidList("mail");
if(forbidList != null){
...
<td width="10%"><a href="forbuser2.jsp?delete=<%=forbid.getValue()%>">删除</a></td>
</tr>
<%
	    i ++;
	}
