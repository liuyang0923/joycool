...
function moveUpSelect(select){
	var s = select.options.selectedIndex;
	if(s>0){
		select.options[s].swapNode(select.options[s-1]);
	}
}
function moveDownSelect(select){
	var s = select.options.selectedIndex;
	if(s<select.options.length-1&&s>=0){
		select.options[s].swapNode(select.options[s+1]);
	}
}
