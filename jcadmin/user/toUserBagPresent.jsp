...
String prefixUrl = "toUserBagPresent.jsp?userId="+userId;
String fenye = null;
...
if(nickname!=null)
{
	...
	{
	String condition = "to_user_id="+user.getId() + " order by time desc " + " limit " + pageIndex
				* NUMBER_PER_PAGE + ", " + NUMBER_PER_PAGE;
	userBagItem = service.getUserBagItemList(condition);
	if(userBagItem==null)
	{tip="没有查到与此相匹配的用户。";}
	prefixUrl = "userBagPresent.jsp?userId="+user.getId();
	totalCount = SqlUtil.getIntResult("select count(id) from user_item_log where to_user_id="+user.getId());
	}
}
if(userId>0){
user=UserInfoUtil.getUser(userId);
String condition = "to_user_id="+userId + " order by time desc " + " limit " + pageIndex
				* NUMBER_PER_PAGE + ", " + NUMBER_PER_PAGE;
	     userBagItem = service.getUserBagItemList(condition);
	     totalCount = SqlUtil.getIntResult("select count(id) from user_item_log where to_user_id="+userId);
		if(userBagItem==null) {
			tip="此用户没有赠送记录!";
		}
}
...
<html>
<div align="center">
<H2 align="center">接收赠送道具管理后台</H2>
<a href="/jcadmin/user/userBagPresent.jsp?userId=<%=userId%>">根据赠送者查询</a>
<%if(user!=null){%>
...
%><tr><%
	...
	UserBean user1=UserInfoUtil.getUser(userItem.getUserId());
...
%>
<td><%if(userItem.getUserId()!=0){%><a href="queryUserInfo.jsp?id=<%=userItem.getUserId()%>"><%=userItem.getUserId()%></a><%}%></td>
<td><%if(user1!=null){%><%=StringUtil.toWml(user1.getNickName())%><%}%></td>
<td><%=bean.getName()%></td>
...
根据ID查找行囊赠送记录<br/>
<form name="form1" method="get" action="toUserBagPresent.jsp">
