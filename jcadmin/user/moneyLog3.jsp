...
if(userId!=0)
{
	...
	user=UserInfoUtil.getUser(userId);
		...
		bankLog=userService.getMoneyLogList("to_id = "+ userId + " order by id desc limit "+paging.getStartIndex()+","+paging.getCountPerPage());
		if(bankLog==null) {
	    tip="此用户没有赠送记录!";
	    }
}
...
%>
<td><a href="queryUserInfo.jsp?id=<%=moneyLog.getFromId()%>"><%=moneyLog.getFromId()%></a></td>
...
<%=paging.shuzifenye("moneyLog3.jsp?userId="+userId,true,"|",response)%><br/>
<font color="red"><%=tip!=null?tip:""%></font><br/>
<a href="moneyLog2.jsp?userId=<%=userId%>">根据赠送人查询</a><br/>
根据ID查找赠送记录<br/>
<form name="form1" method="post" action="moneyLog3.jsp">
