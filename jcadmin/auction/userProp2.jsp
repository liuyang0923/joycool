...
if(userId>0   && dummyList.size()>0&&time>0){
	...
	<script>
	alert("用户ID不存在!");
	window.navigate("userProp2.jsp?userId=<%=userId%>");
	</script>
	<%return;
	}
...
for (int i = 0; i < vec.size(); i++) {
	...
	if(bean.getDummyId()<4||bean.getMode()>0)
		...
		;
	else if(bean.getDummyId()<20){
		if(bean.getClass1()==10) {
			if(bean.getId()>700)
				lists[0].add(bean);
			else if(bean.getId()>400)
				lists[1].add(bean);
			else
				lists[2].add(bean);
		} else if(bean.getClass1()==1) {
			lists[3].add(bean);
		} else if(bean.getClass1()==2) {
			if(bean.getClass2()>16)
				lists[4].add(bean);
			else if(bean.getClass2()>14)
				lists[5].add(bean);
			else
				lists[6].add(bean);
		} else if(bean.getClass1()==5)
			lists[7].add(bean);
		else
			lists[8].add(bean);
	}
...
function checkform(){
	...
	<form action="userProp2.jsp" name="userProp" method="post" onsubmit="return checkform()">
...
<table valign=top width="1400"><tr>
