...
List list = SqlUtil.getIntsList("select user_id,tong_id,count,mark from jc_tong_city_record where tong_id=" + tong.getId() + " and mark=1 order by count desc limit " + start + ",20");
...
int[] res = (int[])list.get(i);
UserBean user = UserInfoUtil.getUser(res[0]);
...
%>
	<tr>	
		...
		<td><%=res[2]%></td>
		<%if(user2!=null){%><td><%=user2.getIpAddress()%></td><td><%=user2.getUserAgent()%></td><%}else{%><%}%>
	</tr>
<%}%>
	</table>
	<br /><%=paging.shuzifenye("wall.jsp?tongId="+tongId,true,"|",response)%><br/>
<form  action="wall.jsp" method="post"  name="tongForm"><br/>
帮会ID：
