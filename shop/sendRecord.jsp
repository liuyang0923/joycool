...
response.setHeader("Cache-Control", "no-cache");
	...
	String condition = " uid = " + uid + " and user_id > 0 and type in(0,1) order by time desc";
...
<a href="record.jsp">购买</a>.赠送.<a href="receiveRecord.jsp">收礼</a>.<a href="/pay/myOrder.jsp">充值</a><br/>
您一共送出了<%=count %>件商品<br/>
<%
for(int i = 0; i < list.size() ; i ++) {
		GoldRecordBean recordBean = (GoldRecordBean)list.get(i);
		UserBean user = UserInfoUtil.getUser(recordBean.getUserId());
...
%>
赠送给<a href="/home/home.jsp?userId=<%=recordBean.getUserId() %>"><%=user.getNickNameWml() %></a>一个【<a href="item.jsp?id=<%=recordBean.getItemId()%>"><%=productBean.getName() %></a>】.<%=ShopUtil.GOLD_NAME %><%=recordBean.getGoldString() %><br/>
<%=DateUtil.formatDate2(recordBean.getTime()) %><br/>
<%}%>
<%=paging.shuzifenye("sendRecord.jsp", false,"|", response) %>
