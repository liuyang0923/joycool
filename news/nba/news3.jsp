<%@ page contentType="text/vnd.wap.wml;charset=utf-8"%><%@ page import="net.joycool.wap.framework.BaseAction,jc.news.nba.*,java.util.*,net.joycool.wap.bean.*,net.joycool.wap.util.*"%><%
	...
	List ml = action.service.getMatchList(" del=0 and (team1 like'"+ StringUtil.toSqlLike("'湖人'")+ "' or team2 like'"+ StringUtil.toSqlLike("'湖人'")
				+ "') and start_time > now() and del=0 order by start_time limit 5");
	List list = action.service.getNews(" flag=3 order by create_time desc");
...
if(ml != null && ml.size() > 0){
	...
	for(int i=paging.getStartIndex();i<=paging.getEndIndex()-1;i++){
		...
		if(bm.getTeam1().contains("湖人")){
		%>VS<%=bm.getTeam2()%><br/><%
		}else{
		%>VS<%=bm.getTeam1()%><br/><%
		}
	}
	%><%=paging.shuzifenye("news3.jsp?jcfr=1", true, "|", response)%><%
}
%>=球队新闻=<br/><%
