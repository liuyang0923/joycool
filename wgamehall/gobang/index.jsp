...
//当前游戏
WGameHallBean currentGoBang = (WGameHallBean)request.getAttribute("currentGoBang");
...
<wml>
<card title="五子棋" ontimer="<%=response.encodeURL(refreshUrl)%>">
<timer value="200"/>
<p align="center">五子棋</p>
<p align="left">
<%=BaseAction.getTop(request, response)%>
<img src="../img/gobang.gif" alt="五子棋"/><br/>
+玩家列表+<br/>
<%
count = userList.size();
for(i = 0; i < count; i ++){
...
%>
<%=(pageIndex1 * action.ONLINE_NUMBER_PER_PAGE + i + 1)%>.<%if(user.getUs2()!=null){%><%=user.getUs2().getHatShow()%><%}%><%=StringUtil.toWml(user.getNickName())%>(<%=user.getUs().getGamePoint()%>乐币)<a href="/wgamehall/gobang/invite.jsp?userId=<%=user.getId()%>">邀请</a><br/>
<%
}
...
if(currentGoBang != null){
	...
	if(loginUser.getId() == currentGoBang.getLeftUserId()){
		us=UserInfoUtil.getUserStatus(currentGoBang.getRightUserId());
		if(us!=null)
		enemyNickname = us.getHatShow()+StringUtil.toWml(currentGoBang.getRightNickname());
		else
			enemyNickname=StringUtil.toWml(currentGoBang.getRightNickname());
			
	} else {
		us=UserInfoUtil.getUserStatus(currentGoBang.getLeftUserId());
		if(us!=null)
			enemyNickname = us.getHatShow()+StringUtil.toWml(currentGoBang.getLeftNickname());
		else
			enemyNickname=StringUtil.toWml(currentGoBang.getLeftNickname());
			
	}
%>
<a href="/wgamehall/gobang/playing.jsp?gameId=<%=currentGoBang.getId()%>">你正和<%=StringUtil.toWml(enemyNickname)%>下棋</a><br/>
<%
}
count = hallList.size();
for(i = 0; i < count; i ++){
...
%>
+功能选项+<br/>
<a href="/wgamehall/gobang/help.jsp">游戏帮助</a>|<a href="/wgamehall/gobang/history.jsp">今日战绩</a><br/>
<br/>
