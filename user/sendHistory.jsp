...
action.sendHistory(request,response);
...
if(loginUser!=null){
	if(recordList!=null && recordList.size()>0){
		...
		for(int i=0;i<recordList.size();i++){
			...
			user=action.getUser(String.valueOf(record.getToId()));
			toUserName=StringUtil.toWml(user.getNickName());
			if(user.getUs2()!=null){
				toUserName=user.getUs2().getHatShow()+toUserName;
			}
			content=StringUtil.toWml(action.getRankAction(record.getActionId()).getSendMessage());
			content=content.replace("XXX","<a href=\"/user/ViewUserInfo.do?userId="+record.getToId()+"\">"+toUserName+"</a>");
			//mcq_end
...
<%}%>
<br/>
<a href="/user/receiveHistory.jsp?backTo=http://wap.joycool.net/user/messageIndex.jsp">我收到的动作</a><br/>
