...
List tongCityRecordByIdList=(List)request.getAttribute("tongCityRecordByIdList");%>
<card title="乐酷">
<p align="left">
<%=BaseAction.getTop(request, response)%>
<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=count">按次数</a>/<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=create_datetime">按时间</a>/按帮会<br/>
<%
TongCityRecordBean tongCiytRecord = null;
TongBean tong1=null;
for(int i=0;i<tongCityRecordByIdList.size();i++){
    tongCiytRecord=(TongCityRecordBean)tongCityRecordByIdList.get(i);
...
<%}else{
Vector tongCiytRecordList=(Vector)request.getAttribute("tongCiytRecordList");
%>
<card title="乐酷">
<p align="left">
<%=BaseAction.getTop(request, response)%>
<%if(orderBy.equals("create_datetime")){%>
<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=count">按次数</a>/按时间<%--/<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=tongCityTongId">按帮会</a>--%><br/>
<%}else{%>
按次数/<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=create_datetime">按时间</a><%--/<a href="/tong/tongCityRecord.jsp?tongId=<%=tong.getId()%>&amp;orderBy=tongCityTongId">按帮会</a>--%><br/>
<%}
TongCityRecordBean tongCiytRecord = null;
UserBean user=null;
for(int i=0;i<tongCiytRecordList.size();i++){
    tongCiytRecord=(TongCityRecordBean)tongCiytRecordList.get(i);
   	user=UserInfoUtil.getUser(tongCiytRecord.getUserId());
   	if(user==null){continue;}
   	%>
<%if(user.getId()!=loginUser.getId()){%><a href="/user/ViewUserInfo.do?userId=<%=user.getId()%>"><%=StringUtil.toWml(user.getNickName()) %></a><%}else{%>您自己<%}%>
<%=tongCiytRecord.getCount()%>次<br/>
<%}String fenye = PageUtil.shuzifenye(totalPageCount, pageIndex, prefixUrl, true, "|", response);
if(!"".equals(fenye)){%><%=fenye%><%}%><br/>
<a href="/tong/tongCity.jsp?tongId=<%=tong.getId()%>">帮会城墙</a><br/>
