<%@ page contentType="text/vnd.wap.wml;charset=utf-8"%><%@ page import="net.joycool.wap.framework.BaseAction"%><%@ page import="net.joycool.wap.bean.*" %><%@ page import="net.joycool.wap.util.UserInfoUtil" %><%@ page import="net.joycool.wap.util.StringUtil" %><%@ page import="net.joycool.wap.util.Constants" %><%@ page import="net.joycool.wap.util.PageUtil" %><%@ page import="java.util.List" %><%@ page import="net.joycool.wap.action.dhh.*" %><%
...
----------------<br/>
<%if(dhhUser!=null){%>
你最近高分为：<%=dhhUser.getRecentHighScore()%>￥，排名第<%=request.getAttribute("count")%>。<br/>
<%}%>
<%
...
for(int i =0 ;i<fsUserList.size();i++){
	...
	<%}%>
	最高分:<%=fsUser1.getRecentHighScore()%>￥<br/>
	
<%}%>
<%=PageUtil.shuzifenye(pagingBean, "top.jsp", false, "|", response)%>
<%if(!dhhUser.isGameOver()){%>
<a href="/dhh/play.jsp">返回游戏</a><br/>
<%}%>
<a href="/dhh/topall.jsp">查看总排行榜</a><br/>
