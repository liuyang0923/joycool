...
%>
<%=tip%><br/>
<a href="/wgamepk/basketball/index.jsp">我坐庄的赌局</a><br/>
<%
} else if("continue".equals(result)){
%>
提醒：坐庄后将无法取消，请慎重决定。如果无人挑庄，您的庄将在72小时后自动撤掉.<br/>
下注(至少10个乐币,根据篮球规则将暂扣下注额两倍的乐币,您还有<%=us.getGamePoint()%>个乐币):<br/>
...
<anchor title="左">突破上篮   
    <go href="/wgamepk/basketball/bkDeal1.jsp" method="post">
    <postfield name="wager" value="$wager"/>
	<postfield name="action" value="l"/>
    </go>
</anchor>
<br/>
<anchor title="中">三分远投<br/>
    <go href="/wgamepk/basketball/bkDeal1.jsp" method="post">
    <postfield name="wager" value="$wager"/>
	<postfield name="action" value="m"/>
    </go>
</anchor>
<anchor title="右">飞身扣篮<br/>
    <go href="/wgamepk/basketball/bkDeal1.jsp" method="post">
    <postfield name="wager" value="$wager"/>
