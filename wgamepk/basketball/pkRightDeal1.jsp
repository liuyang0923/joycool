<%@ page contentType="text/vnd.wap.wml;charset=utf-8"%><%@ page import="net.joycool.wap.action.wgamepk.ResultPicture"%><%@ page import="net.joycool.wap.framework.BaseAction"%><%@ page import="net.joycool.wap.util.*"%><%@ page import="net.joycool.wap.bean.wgame.*"%><%@ page import="net.joycool.wap.action.wgamepk.*"%><%@ page import="net.joycool.wap.action.wgame.WGameDataAction"%><%@ page import="net.joycool.wap.bean.*"%><%@ page import="net.joycool.wap.util.StringUtil"%><%
...
<img src="/wgamepk/basketball/img/no.gif" alt="射门右vs守门右"/><br/>
    ...
    if(basketball.getWinUserId() == basketball.getLeftUserId()){
        //加入谣言
//	    WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("/wgamepk/basketball/index.jsp") + "\" title=\"go\">射门</a>]输给" + basketball.getLeftNickname() + "*" + basketball.getWager() + "个乐币");
	    UserStatusBean usTemp=null;
	    usTemp=UserInfoUtil.getUserStatus(basketball.getLeftUserId());
%>
扑救失败,您输给<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(basketball.getLeftNickname())%><%=basketball.getWager()%>个乐币!您现在还有<%=us.getGamePoint()%>个乐币<br/>
<%
	} else if(basketball.getWinUserId() == basketball.getRightUserId()){
        //加入谣言
//	    WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("/wgamepk/basketball/index.jsp") + "\" title=\"go\">射门</a>]赢了" + basketball.getLeftNickname() + "*" + basketball.getWager() + "个乐币");
	    UserStatusBean usTemp=null;
	    usTemp=UserInfoUtil.getUserStatus(basketball.getLeftUserId());
%>
扑救成功,您赢了<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(basketball.getLeftNickname())%><%=basketball.getWager()*2%>个乐币!您现在还有<%=us.getGamePoint()%>个乐币<br/>
<%
	}
...
%>
<a href="/wgamepk/basketball/pkStart.jsp?userId=<%=basketball.getLeftUserId()%>">邀请对方再PK一局</a><br/>
<br/>
