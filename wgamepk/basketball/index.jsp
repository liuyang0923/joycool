...
<wml>
<card title="篮球飞人" ontimer="<%=response.encodeURL(refreshUrl)%>">
<timer value="200"/>
<p align="center">篮球飞人</p>
<p align="left">
<%=BaseAction.getTop(request, response)%>
<img src="./img/basketball.gif" alt="射门"/><br/>
+庄家列表+<a href="/wgame/basketball/index.jsp">挑战美女</a><br/>
<%
count = bkList.size();
for(i = 0; i < count; i ++){
	...
	if(loginUser==null)
	{
%><%=(pageIndex * action.BK_NUMBER_PER_PAGE + i + 1)%>.<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(bk.getLeftNickname())%>坐庄<%=bk.getWager()%>乐币:<a href="/wgamepk/basketball/pkout.jsp?bkId=<%=bk.getId()%>">挑战</a><br/>	<%
	}
	else{
...
%>
<a href="/wgamepk/basketball/bkStart.jsp">我要坐庄&gt;&gt;</a><br/>
+玩家列表+<br/>
<%
count = userList.size();
for(i = 0; i < count; i ++){
...
%>
<%=(pageIndex1 * action.ONLINE_NUMBER_PER_PAGE + i + 1)%>.<%if(user.getUs2()!=null){%><%=user.getUs2().getHatShow()%><%}%><%=StringUtil.toWml(user.getNickName())%>(<%=user.getUs().getGamePoint()%>乐币)<a href="/wgamepk/basketball/pkStart.jsp?userId=<%=user.getId()%>">挑战</a><br/>
<%
}
...
%>
+功能选项+<br/>
<a href="/wgamepk/basketball/help.jsp">游戏帮助</a>|<a href="/wgamepk/basketball/history.jsp">今日战绩</a><br/>
<br/>
