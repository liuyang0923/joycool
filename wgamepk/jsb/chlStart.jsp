...
<% //赢了
  if(jsb.getWinUserId() == jsb.getLeftUserId()){
    //加入谣言
//	WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("index.jsp") + "\" title=\"go\">剪刀石头布</a>]输给" + jsb.getLeftNickname() + "*" + jsb.getWager() + "个乐币");
...
<%
    ...
    //加入谣言
//	WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("index.jsp") + "\" title=\"go\">剪刀石头布</a>]赢了" + jsb.getLeftNickname() + "*" + jsb.getWager() + "个乐币");
	UserStatusBean usTemp=null;
    usTemp=UserInfoUtil.getUserStatus(jsb.getLeftUserId());
%>
...
%>
您和庄家<%=StringUtil.toWml(jsb.getLeftNickname())%>打平了!您现在还有<%=us.getGamePoint()%>个乐币<br/>
<%
	}
}
...
<a href="pkStart.jsp?userId=<%=jsb.getLeftUserId()%>">邀请对方再PK一局</a><br/>
<%}%>
<br/>
