...
<wml>
<card title="三公" ontimer="<%=response.encodeURL(refreshUrl)%>">
<timer value="200"/>
<p align="center">三公</p>
<p align="left">
<%=BaseAction.getTop(request, response)%>
<img src="../img/3gong.gif" alt="三公"/><br/>
+庄家列表+<a href="/wgame/3gong/index.jsp">挑战美女</a><br/>
<%
count = bkList.size();
for(i = 0; i < count; i ++){
	...
	if(loginUser==null)
	{
%><%=(pageIndex * action.BK_NUMBER_PER_PAGE + i + 1)%>.<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(bk.getLeftNickname())%>坐庄<%=bk.getWager()%>乐币:<a href="/wgamepk/3gong/chlStart.jsp?bkId=<%=bk.getId()%>">挑战</a><br/>	<%
	}
	else{
...
%>
<a href="/wgamepk/3gong/bkStart.jsp">我要坐庄&gt;&gt;</a><br/>
+玩家列表+<br/>
<%
count = userList.size();
for(i = 0; i < count; i ++){
...
%>
<%=(pageIndex1 * action.ONLINE_NUMBER_PER_PAGE + i + 1)%>.<%if(user.getUs2()!=null){%><%=user.getUs2().getHatShow()%><%}%><%=StringUtil.toWml(user.getNickName())%>(<%=user.getUs().getGamePoint()%>乐币)<a href="/wgamepk/3gong/pkStart.jsp?userId=<%=user.getId()%>">挑战</a><br/>
<%
}
...
%>
+功能选项+<br/>
<a href="/wgamepk/3gong/help.jsp">游戏帮助</a>|<a href="/wgamepk/3gong/history.jsp">今日战绩</a><br/>
<br/>
