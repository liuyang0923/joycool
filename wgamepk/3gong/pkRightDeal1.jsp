...
<%
    ...
    if(gong3.getWinUserId() == gong3.getLeftUserId()){
		//加入谣言
//	    WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("/wgamepk/3gong/index.jsp") + "\" title=\"go\">三公</a>]输给" + gong3.getLeftNickname() + "*" + gong3.getWager() + "个乐币");
...
<%
	    ...
	    //加入谣言
	     UserStatusBean usTemp=null;
	    usTemp=UserInfoUtil.getUserStatus(gong3.getLeftUserId());
//	    WGameDataAction.addRumor("<a href=\"" + ("/wgame/hall.jsp") + "\" title=\"go\">赌场</a>:<a href=\"" + ("/chat/post.jsp?to=" + loginUser.getUserName()) + "\" title=\"go\">" + StringUtil.toWml(loginUser.getNickName()) + "</a>[<a href=\"" + ("/wgamepk/3gong/index.jsp") + "\" title=\"go\">三公</a>]赢了" + gong3.getLeftNickname() + "*" + gong3.getWager() + "个乐币");
%>
您的牌点数大,您赢了<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(gong3.getLeftNickname())%><%=gong3.getWager()%>个乐币!您现在还有<%=us.getGamePoint()%>个乐币<br/>
<%
	} else {
%>
您和庄家<%=StringUtil.toWml(gong3.getLeftNickname())%>打平了!您现在还有<%=us.getGamePoint()%>个乐币<br/>
<%
	}
if(chatUrl != null){
%>
