...
//结束
	...
	int[] rightDices = pk.getRightDices();
	UserStatusBean usTemp=null;
    usTemp=UserInfoUtil.getUserStatus(pk.getLeftUserId());
    UserStatusBean usRight=UserInfoUtil.getUserStatus(pk.getRightUserId());
int[] leftDices = pk.getLeftDices();
%>
<%--
<%
...
<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(pk.getLeftNickname())%>的骰子:<br/>
...
<%if(usRight!=null){%><%=usRight.getHatShow()%><%}%><%=StringUtil.toWml(pk.getRightNickname())%>的骰子:<br/>
<%
	if(rightDices != null){
%>
...
%>
<a href="/wgamepk/dice/pkStart.jsp?userId=<%=enemyId%>">邀请对方再PK一局</a><br/>
<%
}
%>
<br/>
