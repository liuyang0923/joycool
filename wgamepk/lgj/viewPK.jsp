...
<%if(usTemp!=null){%><%=usTemp.getHatShow()%><%}%><%=StringUtil.toWml(pk.getLeftNickname())%>手式:<br/>
<img src="/wgame/lgj/img/<%=a %>.gif" alt="<%=action.getName(a)%>"/><br/>
<%if(usRight!=null){%><%=usRight.getHatShow()%><%}%><%=StringUtil.toWml(pk.getRightNickname())%>手式:<br/>
<%
	if(b != null){
%>
...
%>
<a href="/wgamepk/lgj/pkStart.jsp?userId=<%=enemyId%>">邀请对方再PK一局</a><br/>
<%
}
%>
<br/>
