...
list = YardAction.yardService.getYardRecipeProtoBeanList("`type`=0 and rank<=" + (yard.getRank()+1)+" order by rank,id");
...
<%if ("".equals(tip)){
%><a href="shop.jsp?id=<%=fmId%>">调味品</a>|食谱|<a href="shop5.jsp?t=1&amp;id=<%=fmId%>">中级食谱</a><br/><%
int iterator = 1;
		...
		for (int i = paging.getStartIndex() ; i < paging.getEndIndex() ; i++){
			...
			if (recipe != null){
				if (yard.isRecipeSet(recipe.getId()) || yard.isMiddleRecipeSet(recipe.getId())){
					...
					%><%=iterator%>.<a href="shop4.jsp?id=<%=id%>&amp;b=1&amp;bid=<%=recipe.getId()%>">[<%=recipe.getName()%>]</a><%if(recipe.getRank()>yard.getRank()){%>等级不足<%}else{%><%=recipe.getRank()+1%>级<%}%><%
				}
				++iterator;
				%><br/><%
			}
		}%><%=paging.shuzifenye("shop3.jsp?id=" + id + "&amp;t=1",true,"|",response)%><%
}
} else {
%><%=tip%><br/><%
}%>
