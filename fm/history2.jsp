...
<a href="history.jsp?id=<%=id%>">家族历史</a>|成员变动记录<br/><%
int c=action.service.selectIntResult("select count(id) from fm_user_log where fm_id=" + id);
PagingBean paging= new PagingBean(action, c, 10, "p");
List list=action.service.selectFmUserLogList("fm_id="+id+" order by id desc limit "+paging.getStartIndex()+","+paging.getCountPerPage());
...
<%=paging.shuzifenye("history2.jsp?id=" + id, true, "|", response)%>
