...
//	HomePlayer player = homeService.getPlayer(" user_id = " + uid);
	...
	HomeUserBean homeUser = HomeCacheUtil.getHomeCache(uid);			//"我"的家园用户信息
...
我的家园|<%if (user.getGender()==0){%><a href="home2.jsp?userId=<%=coupleUid%>&amp;b=1">老公的家园</a><%}else{%><a href="fehome2.jsp?userId=<%=coupleUid%>&amp;b=1">老婆的家园</a><%}%><br/>
...
<%if (jc.match.MatchAction.getMatchUser(uid)==null){
	...
	%><a href="/friend/match/vote.jsp?uid=<%=uid%>">为我投票</a><br/><%	
}%>
...
if (coupleUser != null){
	photo = homeService.getHomePhoto(" id=" + coupleUser.getRecommended());
	if (photo != null){
