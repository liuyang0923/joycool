<%@ page contentType="text/vnd.wap.wml;charset=utf-8"%><%@ page language="java" import="java.util.*" pageEncoding="utf-8"%><%@ page import="net.joycool.wap.util.*,net.joycool.wap.service.factory.*,net.joycool.wap.bean.*,net.joycool.wap.cache.*,net.joycool.wap.action.money.*,net.joycool.wap.cache.util.*,net.joycool.wap.action.user.*,net.joycool.wap.action.home.*,net.joycool.wap.bean.friend.FriendMarriageBean"%><%@ page import="net.joycool.wap.service.infc.*,net.joycool.wap.spec.buyfriends.*"%><%@ page import="net.joycool.wap.framework.BaseAction, net.joycool.wap.action.friend.FriendAction,net.joycool.wap.bean.home.*"%><%@ page import="net.joycool.wap.spec.friend.*"%><%!
...
//	HomePlayer player = homeService.getPlayer(" user_id = " + uid);		//参赛用户
	
	// ==========临时投票==========
//	int voted = homeAction.getParameterInt("v");
//	if (voted == 1 && player != null){
//		HomePlayerVote playerVote = new HomePlayerVote();
//		playerVote.setFromUid(homeAction.getLoginUser().getId());
//		playerVote.setToUid(uid);
//		playerVote.setDel(0);
//		playerVote.setFlag(0);
//		homeService.vote(playerVote);
//	}
	
	int backId = homeAction.getParameterInt("b");
...
%><a href="/friend/match/vote.jsp?uid=<%=uid%>">为她投票</a><br/><%	
}%>
...
<a href="../friend/astro/index.jsp?uid=<%=uid%>">星座配对</a>.<a href="mf.jsp?uid=<%=uid%>">与<%=user.getGenderText()%>交友</a><br/>	
<%if(!flag) {%>
<a href="<%=("/user/OperFriend.do?add=1&amp;friendId="+uid) %>">加为好友</a><br/><%} %>
【最新动态】<% List trendList2 = homeUser.getTrend();
if(trendList2.size() > 5) {%><a href="<%=("userTrend.jsp?uid=" + uid) %>">全部</a><%} else if(trendList2.size() == 0) {%>暂无<%} %><br/>
