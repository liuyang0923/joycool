...
<%	response.setHeader("Cache-Control","no-cache");
	...
	int totalCount = SqlUtil.getIntResult("select count(id) from flower_message where user_id = " + action.getLoginUserId() + " and (type = 1 or type = 2)", 5);
	PagingBean paging = new PagingBean(action,totalCount,COUNT_PRE_PAGE,"p");
	int pageNow = paging.getCurrentPageIndex();
	List logList = service.getMessageList("user_id = " + action.getLoginUserId() + " and (type = 1 or type = 2) order by id desc limit " + pageNow * COUNT_PRE_PAGE + "," + COUNT_PRE_PAGE);
