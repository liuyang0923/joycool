...
<%}else{%>
<%=skill.getName()%>(等级<%=skill.getRank()%>)<br/>
<%=skill.getInfo()%><br/>
<%if(skill.getElement()>0){%>五行(元素):<%=FarmUserBean.elementName[skill.getElement()]%><%}%>
<%if(!skill.isFlagBattle()){%>
材料:<%=FarmWorld.getItemListString(skill.getMaterialList())%><br/>
制造:<%=FarmWorld.getItemListString(skill.getProductList())%><br/>
<%if(cooldown>0){%>
还需要<%=DateUtil.formatTimeInterval(cooldown)%>才能再次使用<br/>
<a href="skill.jsp?id=<%=skill.getId()%>">刷新</a><br/>
<%}else{%>
<a href="skill.jsp?a=1&amp;id=<%=skill.getId()%>">使用</a><br/>
<%}%>
<%}%>
<%}%>
