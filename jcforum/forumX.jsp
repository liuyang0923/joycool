...
<a href="forumX.jsp?forumId=<%=forumId%>">返回论坛</a><br/>
<%}
...
确定要删除该主题吗?<br/>
<a href="forumX.jsp?c=1&amp;forumId=<%=forumId%>&amp;del=<%=con.getId()%>">确认删除</a><br/>
<a href="forumX.jsp?forumId=<%=forumId%>">返回论坛</a><br/>
<%}else if(forum!=null){
...
<%=forum1.getVipTypeStr().length() > 0 ? "["+forum1.getVipTypeStr()+"]":"" %><%if(forum1.isPeak()||forum1.isTopPeak()){%>[顶]<%}else if(forum1.getMark1()==1){%>[精]<%}else{%><%=i+1%>.<%}%>
<a href="viewContentX.jsp?contentId=<%=forum1.getId()%>"><%=StringUtil.toWml(StringUtil.limitString(forum1.getTitle(),30))%></a>(<%if(forum1.getReply()>0){%>回<a href="viewContentX.jsp?r=1&amp;contentId=<%=forum1.getId()%>&amp;forumId=<%=forum1.getForumId()%>"><%=forum1.getReply()%></a><%}%>阅<%=forum1.getCount()%>)
<%      if("super".equals(popedom)){%>|<a href="forumX.jsp?del=<%=forum1.getId()%>&amp;forumId=<%=forum1.getForumId()%>">删除</a><%}%><br/><%}
    }
   }%>
<%=paging.shuzifenye(prefixUrl, true, "|", response)%>
<a href="forumX.jsp?forumId=<%=forumId%>">按时间</a>|
<%if(type!=2){%><a href="forumX.jsp?type=2&amp;forumId=<%=forumId%>">按人气</a><%}else{%>按人气<%}%>|
...
<anchor title="确定">GO
  <go href="forumX.jsp?forumId=<%=forum.getId()%>" method="post">
...
<%}else if(forum.getId()==5545){%>
	<a href="/cast/s.jsp">返回城堡战争首页</a><br/>
<%}%>
    <a href="searchUser.jsp?forumId=<%=forum.getId()%>">搜索</a>|
    <a href="ruleX.jsp?forumId=<%=forum.getId()%>">版规</a>|
...
<select name="nav">
<option onpick="forumX.jsp?forumId=1">乐酷特快</option>
<option onpick="forumX.jsp?forumId=3">情感倾诉</option>
<option onpick="forumX.jsp?forumId=1985">游戏交易</option>
<option onpick="forumX.jsp?forumId=4">随便灌水区</option>
<option onpick="forumX.jsp?forumId=1548">围炉夜话</option>
<option onpick="forumX.jsp?forumId=1802">新人学堂</option>
<option onpick="forumX.jsp?forumId=671">贴图秀场</option>
<option onpick="forumX.jsp?forumId=6">两性夜语</option>
</select>--%><br/>
<%=BaseAction.getAdver(19,response)%>
