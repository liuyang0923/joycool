...
boolean isAdmin = loginUser!=null&&ForbidUtil.isForbid("chata",loginUser.getId());
int id = action.getParameterInt("id");
if(isAdmin&&request.getParameter("del") != null){
	int deleteId = action.getParameterInt("del");
	ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("chat",deleteId);
	if(deleteId<=0||forbid!=null&&forbid.getOperator()!=loginUser.getId()){
		response.sendRedirect("index.jsp");
		return;
	}
    ForbidUtil.deleteForbid("chat",deleteId,loginUser.getId());
    LogUtil.logAdmin(loginUser.getId()+":"+deleteId+":删除聊天封禁");
}
...
<a href="chatfa.jsp">++添加封禁</a><br/>
<%if(id>0){
ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("chat",id);
if(forbid!=null){
...
<%if(isAdmin){%><%if(forbid.getOperator()==loginUser.getId()){%><a href="chatf.jsp?del=<%=forbid.getValue()%>">立即解除</a><%}else{%>(执行人本人才能解除)<%}%><br/><%}%>
<%}}%>
...
%>
<a href="chatf.jsp?id=<%=forbid.getValue()%>"><%=user.getNickNameWml()%></a>
<%if(isAdmin&&forbid.getOperator()==loginUser.getId()){%>-<a href="chatf.jsp?del=<%=forbid.getValue()%>">解除</a><%}%>
<br/>
<%}
}
%>
<%=paging.shuzifenye("chatf.jsp",false,"|",response)%>
<%}%>
<input name="search" format="*N" maxlength="9"/><a href="chatf.jsp?id=$search">按ID搜索</a><br/>
