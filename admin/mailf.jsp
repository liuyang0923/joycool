...
boolean isAdmin = loginUser!=null&&ForbidUtil.isForbid("maila",loginUser.getId());
int id = action.getParameterInt("id");
if(isAdmin&&request.getParameter("del") != null){
	int deleteId = action.getParameterInt("del");
    ForbidUtil.deleteForbid("mail",deleteId,loginUser.getId());
    LogUtil.logAdmin(loginUser.getId()+":"+deleteId+":删除信件封禁");
}
...
<a href="mailfa.jsp">添加封禁</a><br/>
<%if(id>0){
ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("mail",id);
if(forbid!=null){
...
<%if(isAdmin){%><a href="mailf.jsp?del=<%=forbid.getValue()%>">立即解除</a><%}%>
<%}}%>
<a href="mailf.jsp">返回列表</a><br/>
<%}else{%>
<%
...
%>
<a href="mailf.jsp?id=<%=forbid.getValue()%>"><%=user.getNickNameWml()%></a>
<%if(isAdmin){%>-<a href="mailf.jsp?del=<%=forbid.getValue()%>">解除</a><%}%>
<br/>
<%
	    i ++;
	}
...
%>
<%}%>
<input name="search" format="*N" maxlength="9"/><a href="mailf.jsp?id=$search">按ID搜索</a><br/>
