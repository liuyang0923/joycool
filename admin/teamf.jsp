...
boolean isAdmin = loginUser!=null&&ForbidUtil.isForbid("teama",loginUser.getId());
int id = action.getParameterInt("id");
if(isAdmin&&request.getParameter("del") != null){
	int deleteId = action.getParameterInt("del");
    ForbidUtil.deleteForbid("team",deleteId,loginUser.getId());
    LogUtil.logAdmin(loginUser.getId()+":"+deleteId+":删除圈子封禁");
}
...
<a href="teamfa.jsp">添加封禁</a><br/>
<%if(id>0){
ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("team",id);
if(forbid!=null){
...
<%if(isAdmin){%><a href="teamf.jsp?del=<%=forbid.getValue()%>">立即解除</a><%}%>
<%}}%>
<a href="teamf.jsp">返回列表</a><br/>
<%}else{%>
<%
...
%>
<a href="teamf.jsp?id=<%=forbid.getValue()%>"><%=user.getNickNameWml()%></a>
<%if(isAdmin){%>-<a href="teamf.jsp?del=<%=forbid.getValue()%>">解除</a><%}%>
<br/>
<%
	    i ++;
	}
...
%>
<%}%>
<input name="search" format="*N" maxlength="9"/><a href="teamf.jsp?id=$search">按ID搜索</a><br/>
