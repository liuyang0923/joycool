...
boolean isAdmin = loginUser!=null&&ForbidUtil.isForbid("infoa",loginUser.getId());
int id = action.getParameterInt("id");
if(isAdmin&&request.getParameter("del") != null){
	int deleteId = action.getParameterInt("del");
	ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("info",deleteId);
	if(deleteId<=0||forbid!=null&&forbid.getOperator()!=loginUser.getId()){
		response.sendRedirect("index.jsp");
		return;
	}
    ForbidUtil.deleteForbid("info",deleteId,loginUser.getId());
    LogUtil.logAdmin(loginUser.getId()+":"+deleteId+":删除个人资料封禁");
}
...
<a href="infofa.jsp">++添加封禁</a><br/>
<%if(id>0){
ForbidUtil.ForbidBean forbid = ForbidUtil.getForbid("info",id);
if(forbid!=null){
...
<%if(isAdmin){%><%if(forbid.getOperator()==loginUser.getId()){%><a href="infof.jsp?del=<%=forbid.getValue()%>">立即解除</a><%}else{%>(执行人本人才能解除)<%}%><br/><%}%>
<%}}%>
...
%>
<a href="infof.jsp?id=<%=forbid.getValue()%>"><%=user.getNickNameWml()%></a>
<%if(isAdmin&&forbid.getOperator()==loginUser.getId()){%>-<a href="infof.jsp?del=<%=forbid.getValue()%>">解除</a><%}%>
<br/>
<%
	}
}
%><%=paging.shuzifenye("infof.jsp", false, "|", response)%>
<%}%>
<input name="search" format="*N" maxlength="9"/><a href="infof.jsp?id=$search">按ID搜索</a><br/>
